...
local Windows = Fluent:CreateWindow(
    {
        ...
        TabWidth = 130,
        Size = UDim2.fromOffset(workspace.CurrentCamera.ViewportSize.X / 3, workspace.CurrentCamera.ViewportSize.Y / 2),
        Theme = "Darker",
        Acrylic = true,
...
Fluent.Database =
{
    ...
    Macro =
    {
        ...
        Create = function()
            ...
            local s, e = pcall(
                function()
                    assert(isfile, "The excutor doesn't support isfile")
                    assert(writefile, "The excutor doesn't support writefile")
        ...
        Delete = function()
            ...
            local s, e = pcall(
                function()
                    ...
                    assert(isfile, "The excutor doesn't support isfile")
                    assert(delfile, "The excutor doesn't support delfile")
        ...
        Export = function()
            ...
            local s, e = pcall(
                function()
                    assert(n, "Select macro file first")
                    assert(isfile, "The excutor doesn't support isfile")
                    assert(readfile, "The excutor doesn't support readfile")
                    assert(isfile(Otherfunction:FormatLocation(n)), "Can't find selected file")
...
if game.PlaceId == 16146832113 then
    ...
    function EquipUnits()
        ...
        local s, e = pcall(
            function()
                ...
                assert(isfile, "The excutor doesn't support isfile")
                assert(delfile, "The excutor doesn't support delfile")
                assert(isfolder, "The excutor doesn't support isfolder")
                assert(readfile, "The excutor doesn't support readfile")
                assert(delfolder, "The excutor doesn't support delfolder")
                assert(listfiles, "The excutor doesn't support listfiles")
                assert(writefile, "The excutor doesn't support writefile")
                assert(makefolder, "The excutor doesn't support makefolder")
    ...
    Fluent.Database.Signals["Fluent WindowsSize"] = workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(
        function()
            Windows.Root.Size = UDim2.fromOffset(workspace.CurrentCamera.ViewportSize.X / 3, workspace.CurrentCamera.ViewportSize.Y / 2)
            ...
            if Fluent.Unloaded or not Configs["Macro Play"].Value then
                ...
                if not isfile then
                    Fluent:Notify(
                        {
                            Title = "Notification",
                            SubContent = "The excutor doesn't support isfile",
                            Disable = true,
                            Duration = 5
                        }
                    )
                ...
                elseif not readfile then
                    Fluent:Notify(
                        {
                            Title = "Notification",
                            SubContent = "The excutor doesn't support readfile",
                            Disable = true,
                            Duration = 5
                        }
                    )
